...
class ConstructionEnvironment(Env):
    ...
    def step(self, action):
        ...
        self.construction.perform_action_continuous(action, self.boundary_radius)
    ...
    def _point_to_image_space(point: Union[Point, np.array], boundary_radius: int, resolution: int) -> np.array:
        ...
        if type(point) is Point:
            point = point.numpy()
        return (point * resolution / (2 * boundary_radius) + origin).round().astype(np.int16)

    @staticmethod
    def _points_to_action_continuous(point1: np.array, point2: np.array, is_line: bool, boundary_radius: int):
        # If either of the points do not exist, return 0 action
        if point1 is None or point2 is None:
            return 0
        # Otherwise, convert to image (pixel) space coordinates
        point1 = ConstructionEnvironment._point_to_image_space(point1, boundary_radius, 2)
        point2 = ConstructionEnvironment._point_to_image_space(point2, boundary_radius, 2)
        is_line = np.array([is_line], dtype=np.int16)

        # TODO: Figure out how to encode is_line
        action = np.concatenate([point1, point2, is_line])
        return action
            ...
            for point2 in self.construction.points:
                if point1 is not point2:
                    ...
                    if Line(point1, point2) not in self.construction.lines:
                        legal_moves.add(tuple(self._points_to_action_continuous(point1, point2, True, self.boundary_radius)))
                    if Circle(center=point1, point2=point2) not in self.construction.circles:
                        legal_moves.add(tuple(self._points_to_action_continuous(point1, point2, False, self.boundary_radius)))
    ...
    def action_to_string(self, action_number):
        ...
        """
        is_line, point1, point2 = self.construction._interpret_action_continuous(action_number, self.boundary_radius)
        if is_line:
