...
class TestFastPoint(TestPoint):
    ...
    def test_FastPoint_init(self):
        # array init
        assert Point(array=np.array((3, 2)))
        self.assertRaises(ValueError, lambda: Point(array=np.array((3, 2, 3, 2))))  # Wrong shape array

        # string init
        assert Point(x='3', y='10') == Point(x=3, y=10)
